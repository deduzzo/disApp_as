export default {
	...
	async getDistrettiMap() {
		...
		distretti.forEach(d => {
			this.distrettiMap.byUnique[d.uniquetxt] = d;
			this.distrettiMap.byId[d.id] = d;
		});
	},
	getDistrettiOptions() {

		if (this.userData.distretto === undefined || this.userData.distretto === null || this.userData.distretto === "" ) {
			...
			return getAllDistretti.data
				.filter((obj) => obj.id.includes(this.userData.distretto.split(",")))
				.map((obj) => {
				return { label: obj.nome, value: obj.id };
			});
	},
